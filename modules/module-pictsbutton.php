...
class TB_Picts_Buttons_Module extends Themify_Builder_Component_Module
{
    ...
    {
        return __('Extended Button', 'themify');
    }
                ...
                self::get_tab(array(
                    'n' => array(
                        'options' => array(
                            ...
                            self::get_color_type(array(' em', ' span')),
                            self::get_font_size(array(' em', ' span')),
                            self::get_line_height(array(' em', ' span')),
                            self::get_letter_spacing(array(' em', ' span')),
                            self::get_text_align(),
                            self::get_text_transform(' span'),
                            self::get_font_style(' a'),
                            self::get_text_decoration(array(' em', ' span'), 'text_decoration_regular'),
                    ...
                    'h' => array(
                        'options' => array(
                            ...
                            self::get_font_size(array(' em', ' span'), 'f_s', '', 'h'),
                            self::get_font_style(' a', 'f_st', 'f_w', 'h'),
                            self::get_text_decoration(array(' em', ' span'), 't_d_r', 'h'),
        ...
        $button_link = array(
            ...
            self::get_expand('l', array(
                self::get_tab(array(
                    'n' => array(
                        'options' => array(
                            self::get_color(array(' .module-buttons-item a', ' .module-buttons-item a span', ' .module-buttons-item a em'), 'link_color'),
                            self::get_text_decoration(array(' .module-buttons-item a', ' .module-buttons-item a em', ' .module-buttons-item span'))
                        )
                    ),
                    'h' => array(
                        'options' => array(
                            self::get_color(array(' .module-buttons-item a:hover', ' .module-buttons-item a:hover span', ' .module-buttons-item a:hover em'), 'link_color_hover', null, null, ''),
                            self::get_text_decoration(array(' .module-buttons-item a:hover', ' .module-buttons-item a:hover em', ' .module-buttons-item:hover span'), 't_d_h', '')
                        )
                    )
                ))
            )),
    ...
    {
?>
        <# let downloadLink=data.download_link=='yes' ? ' download' : '' ; display=data.fullwidth_button?'':data.display, arr=data.content_button || []; #>
            <div class="module module-<?php echo $this->slug; ?> {{ data.css_button }} {{ data.buttons_size!=='normal'?data.buttons_size:'' }} {{ data.buttons_style }} {{ data.buttons_shape!=='normal'?data.buttons_shape:'' }} {{ display }} {{ data.fullwidth_button }}">
                <# for(let i=0;i<arr.length;++i){ let item=arr[i], icon=item.t!='l' ?(item.icon ? api.Helper.getIcon(item.icon).outerHTML:'') : (api.Helper.getLottie(item,'parent').outerHTML || '' ), aligm=item.icon_alignment; #>
                    <div class="module-buttons-item tf_inline_b">
                        <# if ( item.link ) { if(item.title){ downloadLink+=' title="' +item.title+'"'; } let color=!item.button_color_bg || item.button_color_bg=='default' ? 'tb_default_color' : item.button_color_bg, title=!item.title? '' : 'title="' +item.title+'"', id=item.id ? 'id="' + item.id + '"' : '' ; #>
                            <a class="ui builder_button {{ color }}" href="{{ item.link }}" {{downloadLink}} <# print(title)#> {{{id}}}>
                                <# } if ( icon && aligm !='right' ) { #>
                                    <em class="tf_inline_b tf_vmiddle">
                                        <# print(icon)#>
                                    </em>
                                    <# } #>
